...
static int ili9881c_prepare(struct drm_panel *panel)
{
	...
	if (gpio_is_valid(ctx->reset)) {
		if (ctx->reset_flags & GPIO_ACTIVE_LOW) 
		{
			gpio_set_value(ctx->reset, 0);
			printk("ili9881c_unprepare: reset pin was set 0\n");
		}
		else
		{
			gpio_set_value(ctx->reset, 1);
			printk("ili9881c_unprepare: reset pin was set 1\n");
...
static int ili9881c_unprepare(struct drm_panel *panel)
{
    ...
    printk("ili9881c_unprepare: rest pin value should be 1\n");
...
/*	mode = drm_mode_duplicate(panel->drm, &bananapi_default_mode);
	...
	mode = drm_mode_duplicate(panel->drm, &sc20_ili9881c_mode);
	if (!mode) {
		dev_err(&ctx->dsi->dev, "failed to add mode %ux%ux@%u\n",
			sc20_ili9881c_mode.hdisplay,
			sc20_ili9881c_mode.vdisplay,
			sc20_ili9881c_mode.vrefresh);
		printk("ili9881c_get_modes : failed to add mode\n");
...
static int ili9881c_dsi_probe(struct mipi_dsi_device *dsi)
{
    ...
    printk("ili9881c_dsi_probe: Driver Start....\n");
	...
	if (IS_ERR(ctx->power)) {
		...
		printk("ili9881c : Couldn't get our power regulator\n");
		return PTR_ERR(ctx->power);
	}*/

	ctx->reset = 25;
	/*ctx->reset = of_get_named_gpio_flags(&dsi->dev, "reset", GPIO_ACTIVE_LOW);*/
	/*if (IS_ERR(ctx->reset)) {
		dev_err(&dsi->dev, "Couldn't get our reset GPIO\n");
		printk("ili9881c : Couldn't get our reset GPIO\n");
...
static int ili9881c_dsi_remove(struct mipi_dsi_device *dsi)
{
    ...
    printk("ili9881c_dsi_remove: Driver Exit....\n");
